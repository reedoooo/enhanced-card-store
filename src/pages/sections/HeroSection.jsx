...
const BackgroundBeams = () => {
  // Define the SVG paths for the beams here
  const beams = [
      ...
      >
        {beams.map((d, index) => (
          <path
            ...
            strokeOpacity="0.5"
            strokeWidth="0.5"
            className={`beam-animation beam-${index}`}
          />
        ))}
        <defs>
...
const createSliderImages = (cards) => {
...
const createSlider = (cards) => {
  return (
    <Swiper
      ...
      centeredSlides={true}
      loop={true}
      slidesPerView={3} // Adjusted to show multiple slides at once
      spaceBetween={30} // Adjusted to ensure cards are visible within the viewport
      coverflowEffect={{
        rotate: 0,
        stretch: 50, // Adjust stretching to ensure slides have enough space
        depth: 100,
        modifier: 1,
        slideShadows: false, // Disable slide shadows to prevent visual clutter
      }}
      pagination={{ clickable: true }}
      navigation={true}
      modules={[EffectCoverflow, Pagination, Navigation]}
      className="swiper_container"
      // onSlideChange={(swiper) => setActiveCardIndex(swiper.realIndex)}
    >
      {/* <Box
        sx={{
          ...
          justifyContent: 'center',
          alignItems: 'center',
          height: '100%', // Ensure the Box fills the slide height
          boxSizing: 'border-box',
        }}
      > */}
      {createSliderImages(cards)}
      {/* </Box> */}
    </Swiper>
  );
};
const HeroSection = () => {
  ...
  return (
    ...
    >
      <Typography variant="h1" gutterBottom sx={{ zIndex: 10 }}>
      ...
      >
        {createSliderImages(topFiveCards)}
